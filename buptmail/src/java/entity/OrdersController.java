...
public String search() {
        ...
        try {
           if(current != null){
            String sql1 = "select * from buptmail.orders where orders.id  ="+current.getId();
            Query query;
                query = getFacade().getEntityManager().createNativeQuery(sql1, Orders.class);
            Orders order = (Orders) query.getSingleResult(); //获取单个结果
            location = "订单号："+current.getId()+"    "+"当前位置："+order.getLocation();           
            return null;
}
        } catch (Exception e) {
            location="查无此订单信息，请检查订单信息。";
